...
* [`src/cljs/shopping_app_web/`](src/cljs/shopping_app_web/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/shopping_app_web/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/shopping_app_web/`](test/cljs/shopping_app_web/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `shopping-app-web` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/shopping_app_web/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns shopping-app-web.example
  (:require [shopping-app-web.config :as config])
